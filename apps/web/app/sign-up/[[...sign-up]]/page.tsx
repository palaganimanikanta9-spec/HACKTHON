...
import { Sparkles, Loader2, KeyRound, ArrowLeft } from "lucide-react";
...
export default function SignUpPage() {
  ...
  const handleGoogleSignUp = async () => {
    console.log("Google Sign-Up clicked. Clerk loaded:", isLoaded);
    if (!isLoaded || !signUp) {
    ...
    try {
      ...
      setError("Could not initiate Google sign-up. Please check your Clerk dashboard has Google OAuth enabled.");
      setLoading(false);
    }
  };
      ...
      >
        {!pendingVerification ? (
          <>
            ...
            <p className="text-zinc-400 text-sm mb-8">Start saving smarter with your Google account</p>
            ...
            <form onSubmit={handleSignUpSubmit} className="flex flex-col gap-4">
              ...
              >
                {loading ? <Loader2 size={18} className="animate-spin mx-auto text-white" /> : "Sign Up"}
              </button>
            </form>
          ...
          <>
            ...
            <div className="w-12 h-12 rounded-xl bg-violet-600/10 border border-violet-600/20 flex items-center justify-center text-violet-400 mb-6">
              <KeyRound size={24} />
            </div>

            <p className="text-zinc-400 text-sm mb-6 leading-relaxed">
              We&apos;ve sent a verification code to <span className="text-violet-400 font-semibold">{email}</span>. Please enter the code below to verify your account.
            </p>
            ...
            <form onSubmit={handleVerificationSubmit} className="flex flex-col gap-4">
              <div>
                ...
                <input
                  ...
                  className="w-full h-11 px-4 bg-zinc-950 border border-zinc-800 rounded-xl text-white text-center text-base tracking-widest font-bold focus:outline-none focus:border-violet-600 focus:ring-1 focus:ring-violet-600 transition-colors"
                />
              </div>
              <button
                type="submit"
                ...
                className="w-full h-11 bg-violet-600 hover:bg-violet-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold rounded-xl text-sm transition-all duration-200 shadow-lg shadow-violet-600/20"
              >
                {loading ? <Loader2 size={18} className="animate-spin mx-auto text-white" /> : "Verify Code"}
              </button>
            </form>
          </>
        )}
